...
export const MasteryWorkspace: React.FC<{ assessmentId: string; onExit: () => void }> = ({ assessmentId, onExit }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-cf-card border border-cf-border rounded-xl p-8 text-slate-200 mt-6 shadow-2xl">
      <div className="flex justify-between items-center mb-6 pb-4 border-b border-cf-border">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold transition ${
              active
                ...
                : locked
                ? 'border-cf-border text-slate-600 cursor-not-allowed'
                : 'border-cf-border text-slate-400 hover:border-slate-600'
            }`}
          >
    ...
    <div>
      ...
      {assessment.generatedDrills.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 max-h-48 overflow-y-auto pr-1">
          {assessment.generatedDrills.map((d) => (
            <div key={d.id} className="bg-cf-bg border border-cf-border rounded-md px-3 py-2">
      ...
      {!validation.valid && (
        <div className="mb-4">
          ...
          <textarea
            ...
            className="w-full bg-cf-bg border border-cf-border rounded p-3 text-xs font-mono text-slate-200 resize-none focus:outline-cf-accent mb-2"
          />
          <button
            type="button"
            onClick={onAppend}
      ...
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-cf-accent hover:opacity-90 disabled:bg-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed text-black font-semibold rounded transition"
        >
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-right">
          <div className="text-xl font-mono text-cf-accent">{formatTimer(elapsedSeconds)}</div>
      ...
      <div className="space-y-2 mb-6 max-h-96 overflow-y-auto pr-1">
        {drills.map((d) => (
          <button
            ...
            className="w-full text-left grid grid-cols-2 gap-4 bg-cf-bg border border-cf-border hover:border-slate-600 rounded-lg p-4 transition"
          >
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-right">
          {timedSeconds !== null && (
            <div className={`text-xl font-mono ${timeLeft <= 5 ? 'text-red-400' : 'text-cf-accent'}`}>
...
function MasteryCompleteScreen({ assessment, onExit }: { assessment: Assessment; onExit: () => void }) {
  return (
    <div className="text-center py-10">
      <Trophy className="w-12 h-12 text-cf-accent mx-auto mb-4" />
      ...
      <button onClick={onExit} className="px-5 py-2.5 bg-cf-accent hover:opacity-90 text-black font-semibold rounded transition">
        Return to Dashboard
      </button>
    </div>
  );
